...
const Chats = () => {
  ...
  useEffect(() => {
    ...
    const lastActiveRoom = JSON.parse(lastActiveRoomStr as string)
    const roomExists = chatRooms?.some(existingRoom => existingRoom.room_id === lastActiveRoom.room_id);
    if (lastActiveRoom && roomExists) {
      setActiveChatRoom(lastActiveRoom);
    } else if (chatRooms?.length) {
      setActiveChatRoom(chatRooms[0]);
      localStorage.setItem('lastActiveRoom', JSON.stringify(chatRooms[0]));
    }
  }, [chatRooms]);

  useEffect(() => {
    {triggerGetMembers({ tid: tid as string, search: debouncedSearch })}
  }, [debouncedSearch]);
